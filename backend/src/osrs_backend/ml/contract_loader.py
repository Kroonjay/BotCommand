"""Environment contract loader for action/observation space definitions."""
...
# Default contracts directory - can be overridden
_contracts_dir: Path | None = None

# Type aliases for TorchScript compilation
...
def set_contracts_dir(path: str | Path) -> None:
    """Set the contracts directory path."""
    global _contracts_dir
    _contracts_dir = Path(path)


def get_contracts_dir() -> Path:
    """Get the contracts directory path."""
    if _contracts_dir is not None:
        return _contracts_dir
    # Default: look for contracts in parent directories
    current = Path(__file__).parent
    for _ in range(5):  # Search up to 5 levels
        contracts_path = current / "contracts"
        if contracts_path.exists():
            return contracts_path
        current = current.parent
    raise ValueError("Contracts directory not found. Call set_contracts_dir() first.")
